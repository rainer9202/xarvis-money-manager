...
export function MovementDetailModal() {
  ...
  const { data: movement, isLoading: isLoadingMovements } = useMovement(movementId ?? undefined, { enabled: isOpen });
  ...
  const confirmDelete = () => {
    if (!movement) return;
    confirmDestructive('Eliminar movimiento', '¿Eliminar este movimiento? Esta acción no se puede deshacer.', () =>
          ...
          <Pressable onPress={(event) => event.stopPropagation()}>
            <View className="rounded-t-lg border border-neutral-800 bg-neutral-900">
              <View className="flex-row items-center justify-between border-b border-neutral-800 px-4 py-5">
                <Text className="text-2xl font-semibold text-neutral-50">Detalle del movimiento</Text>
              ...
              <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingVertical: 28 }}>
                {movement ? (
                  <MovementDetail
                    ...
                    accountName={
                      accounts?.find((account) => account.id === movement.accountId)?.name ?? 'Cuenta desconocida'
                    }
                    toAccountName={
                      movement.toAccountId
                        ? (accounts?.find((account) => account.id === movement.toAccountId)?.name ?? 'Cuenta desconocida')
                        : undefined
                    }
                    onEdit={editMovement}
